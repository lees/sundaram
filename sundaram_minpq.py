...
def exclude_generator():
    data_tree = min_pq.MinPQ()
    start_value = func(0)
    last_returned = None
    min_border = start_value[0]
    i = 0
    data_tree.push(start_value[0], start_value)

    while True:
        (value,iterator) = data_tree.pop()
        if value == min_border:
            i += 1
            new_iterator = func(i)
            min_border = new_iterator[0]
            data_tree.push(new_iterator[0], new_iterator)

        next_value = func_next(iterator)
        data_tree.push(next_value[0], next_value)
